...
$db = $database->connect();
// Instantiate blog groups object
...
if($_FILES['avatar'])
{
    ...
    $groups->uid = $_POST['uid'];
    $groups->token = $_POST['token'];
    ...
    if($error > 0){
        ...
        if($_FILES['avatar']['type'] == 'image/jpeg' || 
               ...
               $_FILES['avatar']['type'] == 'image/gif'){
                ...
                $directoryName = $server_dir.$upload_dir.$groups->uid;
                ...
                $upload_name = $server_dir.$upload_dir.$groups->uid."/".$random_name;
                ...
                if($success) {
                    ...
                    $groups->picurl = $server_url.$upload_dir.$groups->uid."/".$random_name;

                    //echo "picurl ".$groups->picurl."\n";
                    
                    $groups->updategrouppic();

                    $response = array(
                        "status" => "true",
                        "message" => "File uploaded successfully",
                        "url" => $server_url.$upload_dir.$groups->uid."/".$random_name
                    );
                }
          } else {
            $response = array(
